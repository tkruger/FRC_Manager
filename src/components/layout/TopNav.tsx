...
export function TopNav({ session, robots = [], activeRobotId }: Props) {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 h-14 border-b border-[--color-border] bg-[--color-surface]/95 backdrop-blur-sm flex items-center px-4 gap-4 transition-colors">
        ...
        {MODULE_TABS.map((tab) => {
          ...
          return (
            ...
            >
              {tab.label}
              {active && (
                <span className="absolute bottom-0 inset-x-0 h-0.5 rounded-t-full bg-[--color-primary]" />
              )}
            </Link>
          );
        })}
      ...
      <div className="flex items-center gap-2 ml-auto shrink-0">
        ...
        {/* Notification bell */}
        <button
          className="h-9 w-9 rounded-md flex items-center justify-center text-[--color-text-secondary] hover:bg-[--color-surface-overlay] transition-colors relative"
  ...
  return (
    <div className="flex items-center gap-1.5 rounded-md border border-[--color-border] bg-[--color-surface-raised] px-3 h-9 text-sm">
      ...
      <select
        ...
        value={activeRobotId ?? ""}
        onChange={(e) => {
          // Robot context switching handled by parent via URL/cookie
      ...
      {isRetired && (
        <span className="badge badge-neutral text-xs">Retired</span>
      )}
    </div>
  );
}
    ...
    <div className="relative group">
      ...
      <div className="absolute right-0 top-full mt-1 w-52 rounded-md border border-[--color-border] bg-[--color-surface-raised] shadow-lg py-1 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity z-50">
        ...
        <Link
          ...
          className="flex items-center gap-2 px-3 py-2 text-sm text-[--color-text-primary] hover:bg-[--color-surface-overlay]"
        >
          Profile settings
        </Link>
        <button
